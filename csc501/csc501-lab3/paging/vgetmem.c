	...
	unsigned nbytes;
{
	...
	nbytes = (unsigned int) roundmb(nbytes); //round or truncate address up to size of mblock
	...
	while(p != (struct mblock *) NULL)
	{
                if ( p->mlen == nbytes) 
		{
                        q->mnext = p->mnext;
                        restore(ps);
                        return 4096*NBPG + ( (WORD *)p - (WORD*)(2048*NBPG+proctab[currpid].store*128*NBPG));
                }
